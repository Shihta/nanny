    ...
    jQuery('#btn_ns_search').click(function(){
    	...
    	if (ns_selector.attr('value') != "" && pre_selection != ns_selector.attr('value')) {
	    	...
	    	jQuery.getJSON(apiurl, {
	    		...
	    		jQuery("#NanniesSearchResultTable").append("<td></td><td>姓名</td><td>登記證編號</td><td>技術證號</td><td>保母編號</td><td>性別</td><td>連絡電話</td><td>地址</td><td>年齡</td><td>教育程度</td><td>收托對象</td><td>托育時段</td><td>已收托幼兒數</td><td>在職訓練</td>");
	    		jQuery.each(data, function(i, item) {
	    			var tr = jQuery("<tr><td>" + i + "</td></tr>");
	    			for (i = 0; i<item.length; i++) {
	    				tr.append(jQuery('<td/>', {text:item[i]}));
	    			}
	    			jQuery("#NanniesSearchResultTable").append(tr);
	    		});
	    	});
    	}
    });
